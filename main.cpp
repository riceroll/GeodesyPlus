...
struct Face_tr;
struct Halfedge_tr;
struct Node_tr_trace;
struct Node_tr_cycle;

struct Node_tr {
  int id = -1;

  Halfedge_tr* halfedge = nullptr;
...
struct Halfedge_tr {
  ...
  Node_tr* node = nullptr;
  Edge_tr* edge = nullptr;
  Face_tr* face = nullptr;
  Halfedge_tr* prev = nullptr;
  Halfedge_tr* next = nullptr;
  Halfedge_tr* twin = nullptr;
...
struct Node_tr_trace {
  ...
  Node_tr_trace* left = nullptr;
  Node_tr_trace* right = nullptr;
  Node_tr_trace* up = nullptr;
  Node_tr_trace* down = nullptr;
...
struct Node_tr_cycle {
  ...
  Node_tr_cycle* left = nullptr;
  Node_tr_cycle* right = nullptr;
  Node_tr_cycle* up = nullptr;
  Node_tr_cycle* down = nullptr;

  Eigen::RowVector3d pos() {
    Node_tr* n0 = this->halfedge->node;
    Node_tr* n1 = this->halfedge->twin->node;
...
int main(int argc, char **argv) {
  ...
  vector<Node_tr*> nodes_tr;
  vector<Edge_tr*> edges_tr;
  vector<Face_tr*> faces_tr;
  vector<Halfedge_tr*> halfedges_tr;
  vector<vector<Node_tr_trace*>> nodes_tr_traces;
  set<Node_tr*> nodes_accepted;    // source of heat method
...
//  float time_step;
  ...
  float foo = 0.05;
  float geodesy_bar = 0.1;
  float geodesy_value = 0;
...
//        viewer.data().add_edges(printing_path[i]->pos, printing_path[i+1]->pos, color);
      ...
      case DisplayMode_HalfedgeTracing: {
        ...
        for (auto e : edges_tr) {
          Node_tr *n_a = e->halfedge->node;
          Node_tr *n_b = e->halfedge->twin->node;
        ...
        {  // draw focus
          ...
          switch (type_focus) {
            case TypeFocus_Node:
              viewer.data().add_points(nodes_tr[idx_focus]->pos, color_node);
              break;

            case TypeFocus_Edge:
              viewer.data().add_points(edges_tr[idx_focus]->centroid(), color_node);
              break;

            case TypeFocus_Face:
              viewer.data().add_points(faces_tr[idx_focus]->centroid(), color_node);
              break;

            case TypeFocus_Halfedge:
              viewer.data().add_points(halfedges_tr[idx_focus]->edge->centroid(), color_node);
              viewer.data().add_points(halfedges_tr[idx_focus]->node->pos, color_node);
              break;
  ...
  function<bool (vector<Edge*>, vector<Node*>)> complete_face = [&](vector<Edge*> es, vector<Node*> ns) {
    ...
    if (find_the_other_face(h_a->node, h_c->node, &ns_triplet, &es_triplet)) {
      if (debug) {
        ...
        viewer.data().add_edges(p0, p1, color_blue);
        viewer.data().add_label((p0+p1)/2, to_string(faces.size()));
      }
...
//            if (e->idx != -1) e->spring = "boundary";
    ...
    display_mode = DisplayMode_Halfedge;   // show halfedge mesh
    redraw();
  };
...
// new tracing ===============================================
  ...
  auto triangulate_tr = [&]() {
    ...
    vector<double> xs;
    vector<double> ys;
    vector<vector<Node_tr*>> nodes_tr_mat;
    vector<vector<vector<Edge_tr*>>> edges_tr_mat;    // i_row, i_col =>
    ...
    for (int i_py = 0; i_py < num_py; i_py++) {
      vector<vector<Edge_tr*>> edges_row;
      for (int i_px = 0; i_px < num_px; i_px++) {
        vector<Edge_tr*> edges_square;
        Edge_tr* e0 = new Edge_tr();  // left edge
        Edge_tr* e1 = new Edge_tr();  // top edge
        Edge_tr* e2 = new Edge_tr();  // diagonal edge
    ...
    for (int i_px = 0; i_px < num_px - 1; i_px++ ) {
      for (int i_py = 0; i_py < num_py - 1; i_py++ ) {
        Node_tr* n0 = nodes_tr_mat[i_py][i_px];  // top_left node
        Node_tr* n1 = nodes_tr_mat[i_py][i_px+1];  // top_right node
        Node_tr* n2 = nodes_tr_mat[i_py+1][i_px];  // bottom_left node
        Node_tr* n3 = nodes_tr_mat[i_py+1][i_px+1];  // bottom_right node

        Edge_tr* e0 = edges_tr_mat[i_py][i_px][0];  // left
        Edge_tr* e1 = edges_tr_mat[i_py][i_px][1];  // top
        Edge_tr* e2 = edges_tr_mat[i_py][i_px][2];  // diagonal
        Edge_tr* e3 = edges_tr_mat[i_py+1][i_px][1];  // bottom
        Edge_tr* e4 = edges_tr_mat[i_py][i_px+1][0];  // right
        ...
        if (is_point_in(i_py + 1, i_px) and is_point_in(i_py, i_px + 1)) {  // diagonal in boundary
          ...
          if (is_point_in(i_py, i_px)) {
            ...
            auto* face = new Face_tr();
            auto* h0 = new Halfedge_tr();  // pointing from top_left to bottom_left
            auto* h1 = new Halfedge_tr();  // bottom_left to top_right
            auto* h2 = new Halfedge_tr();  // top_right to top_left
            ...
            halfedges_tr.emplace_back(h2);

            face->halfedge = h0;
            faces_tr.emplace_back(face);
          ...
          if (is_point_in(i_py+1, i_px+1)) {
            ...
            auto* face = new Face_tr();
            auto* h3 = new Halfedge_tr();  // top_right to bottom_left
            auto* h4 = new Halfedge_tr();  // bottom_left to bottom_right
            auto* h5 = new Halfedge_tr();  // bottom_right to top_right
    ...
    auto face_exterior = new Face_tr();
    face_exterior->is_exterior = true;
//    faces_tr.push_back(face_exterior);
    int halfedges_tr_size = halfedges_tr.size();
    for (int i=0; i<halfedges_tr_size; i++) {
      Halfedge_tr* h = halfedges_tr[i];
      if (not h->twin) {
        auto h_twin = new Halfedge_tr();
        h_twin->twin = h;
        h->twin = h_twin;
        h_twin->edge = h->edge;
        ...
        halfedges_tr.push_back(h_twin);
      }
    }

    // create nexts and prevs for halfedges on the boundary
    for (auto h : halfedges_tr) {
      if (not h->next) {
        if (not h->twin) {
        }
        Halfedge_tr* h_n = h->twin;
        while (true) {
          if (not h_n->prev) {
            h->next = h_n;
            h_n->prev = h;
    ...
    // construct edges_tr & nodes_tr
    for (auto h : halfedges_tr) {
      Edge_tr* e = h->edge;
      Node_tr* n = h->node;

      if (count(edges_tr.begin(), edges_tr.end(), e) == 0) {
        edges_tr.push_back(e);
      }

      if ( count(nodes_tr.begin(), nodes_tr.end(), n) == 0 ) {
        nodes_tr.push_back(n);
      }
    }

    // label nodes, edges, faces, halfedges
    label_vector<Node_tr*>(nodes_tr);
    label_vector<Edge_tr*>(edges_tr);
    label_vector<Face_tr*>(faces_tr);
    label_vector<Halfedge_tr*>(halfedges_tr);

    // initialize accepted nodes with boundary
    for (auto h : halfedges_tr) {
      if (h->face->is_exterior) {
        nodes_accepted.insert(h->node);
      }
    }
  ...
  auto convert_to_mesh = [&]() {
    V_tr.resize(nodes_tr.size(), 3);
    F_tr.resize(faces_tr.size(), 3);
    C_tr.resize(nodes_tr.size(), 3);
    ...
    for (auto f : faces_tr) {
      ...
      Node_tr* n0 = f->halfedge->node;
      Node_tr* n1 = f->halfedge->next->node;
      Node_tr* n2 = f->halfedge->prev->node;
      ...
      int id_f = f->id;
      F_tr.row(id_f) << id_n0, id_n1, id_n2;
    }
  ...
  auto compute_geodesic = [&]() {
    ...
    Eigen::VectorXi sources(nodes_accepted.size());
    int i = 0;
    for (auto n : nodes_accepted) {
      sources(i) = n->id;
      i++;
    }
    ...
    for (int i = 0; i < nodes_tr.size(); i++) {
      nodes_tr[i]->geodesic = D(i) / D.maxCoeff();
    }
...
//        break;
    ...
    cout<<"nodes_tr_size: "<<nodes_tr.size()<<endl;
    cout<<"C_tr size: "<<C_tr.rows()<<endl;
    cout<<"V_tr size:"<<V_tr.rows()<<endl;

    for (int i = 0; i < nodes_tr.size(); i++) {
      C_tr(i,0) = nodes_tr[i]->geodesy;
    }
    viewer.data().set_colors(C_tr);
  };

  auto compute_geodesy_line = [&](double geodesy_gap) {
    // TODO: be careful when value equals to the values on the points
    vector<Node_tr_trace*> nodes_tr_geodesy;
    for (auto e : edges_tr) {
      e->node_trace = nullptr;

      Node_tr* n0 = e->halfedge->node;
      Node_tr* n1 = e->halfedge->twin->node;

      double t = (geodesy_gap - n0->geodesy) / (n1->geodesy - n0->geodesy);

      if ((t >= 0) and (t <= 1)) {
        auto n = new Node_tr_trace();
    ...
    auto get_left_right = [&](Halfedge_tr* h, Node_tr_trace* n) {
      if (h->edge->node_trace) {
        Node_tr_trace* n_n = h->edge->node_trace;
    ...
    for (auto e : edges_tr) {
      ...
      if ((t >= 0) and (t <= 1)) {
        ...
        if (geodesy_n < geodesy) {
          nodes_tr_trace.push_back(n);
          nodes_accepted.insert(n->edge->halfedge->node);
          nodes_accepted.insert(n->edge->halfedge->twin->node);
        }
      }
    }
...
//
//      if (n->left) {
//        viewer.data().add_edges(n->left->pos, n->pos, color_blue);
//      }
//      if (n->right) {
//        viewer.data().add_edges(n->right->pos, n->pos, color_blue);
//      }

    }

    nodes_tr_traces.push_back(nodes_tr_trace);
  };

  auto draw_geodesic_lines = [&]() {

    int num_edges = 0;
    for (auto nodes_trace : nodes_tr_traces) {
      for (auto n : nodes_trace) {
        if (n->left) {
          num_edges++;
        }
      }
...
//    Eigen::MatrixXd points(nodes_accepted.size(), 3);
//    Eigen::MatrixXd colors(nodes_accepted.size(), 3);
//    i = 0;
//    for (auto n : nodes_accepted) {
//      points.row(i) = n->pos;
//      colors.row(i) = color_cyon;
//      i++;
//    }
...
//      ImGui::InputFloat("radius_trim", &radius_trim);
      ...
      ImGui::InputFloat("foo", &foo);
      ImGui::InputFloat("geodesy_bar", &geodesy_bar);
...
//          }
      ...
      if (ImGui::Button("boundary") ) {
        ...
        Eigen::MatrixXd points_a(boundary.size(), 3);
        Eigen::MatrixXd points_b(boundary.size(), 3);
        Eigen::MatrixXd color_e(boundary.size(), 3);
      ...
      if (ImGui::Button("geodesicLine")) {
        ...
        geodesy_value += geodesy_bar;
        compute_geodesy_line(geodesy_value);


        geodesic = 0;
        while (geodesic < 1.0) {
          compute_geodesic_line(geodesic, geodesy_value);
          geodesic += foo;
        }
      ...
      switch (display_mode) {
        ...
        case DisplayMode_HalfedgeTracing:
          switch (type_focus) {
            case TypeFocus_Node:
              if (key == GLFW_KEY_H) {
                idx_focus = nodes_tr[idx_focus]->halfedge->id;
                type_focus = TypeFocus_Halfedge;
              }
              break;
            ...
            case TypeFocus_Edge:
              if (key == GLFW_KEY_H) {
                idx_focus = edges_tr[idx_focus]->halfedge->id;
                type_focus = TypeFocus_Halfedge;
              }
              break;
            ...
            case TypeFocus_Face:
              if (key == GLFW_KEY_H) {
                idx_focus = faces_tr[idx_focus]->halfedge->id;
                type_focus = TypeFocus_Halfedge;
              }
              break;

            case TypeFocus_Halfedge:
              Halfedge_tr *h = halfedges_tr[idx_focus];
              switch (key) {
                case GLFW_KEY_N:
                  idx_focus = h->next->id;
                  break;
