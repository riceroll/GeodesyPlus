...
SHAPEOP_INLINE BendingConstraint::BendingConstraint(const std::vector<int> &idI,
  ...
  w_ =  Vector4::Zero();
  w_(0) = cot02 + cot03; // gradient wrt to theta p0
  w_(1) = cot12 + cot13; // gradient wrt to theta p1
  w_(2) = -(cot02 + cot12); // gradient wrt to theta p2
  w_(3) = -(cot03 + cot13); // gradient wrt to theta p3
  // projection of p onto the gradient
  n_ = (p * w_).norm();
...
SHAPEOP_INLINE void BendingConstraint::project(const Matrix3X &positions, Matrix3X &projections) const {
  ...
  if (n_ > 1e-6) {
    for (int i = 0; i < static_cast<int>(idI_.size()); ++i)
      e += w_(i) * positions.col(idI_[i]); //(x1*grad_x1 + x2*grad_x2 + x3*grad_x3 + x4*grad_x4)
    Scalar l = e.norm();
    if (l > 1e-6) {
      e /= l;
...
SHAPEOP_INLINE void BendingConstraint::flip(){
    // modify n_ based on theta...
    n_ = -n_;
}
///////////////////////////////////////////////////////////////////////////////
